    ...
    <v-main class="login-background d-flex flex-column">
      ...
      <v-card class="white-card custom-rounded-top" elevation="0" width="100%">
        <v-card-text class="custom-padding">
          ...
          <v-btn color="#4c8479" rounded="pill" class="login-btn mb-4" block size="x-large" @click="login">
...
export default {
  ...
  methods: {
    ...
    async login() {
      ...
      const response = await axios.get("http://localhost:7000/login?email="+this.email);
    }
  }
}
